...
ALTER TABLE "wallet_accounts" ADD CONSTRAINT "wallet_accounts_user_id_fkey"
...
ALTER TABLE "ledger_entries" ADD CONSTRAINT "ledger_entries_account_id_fkey"
  FOREIGN KEY ("account_id") REFERENCES "wallet_accounts"("id") ON DELETE RESTRICT ON UPDATE CASCADE;

INSERT INTO "wallet_accounts" ("id", "key", "type", "currency", "balance", "version", "updated_at")
VALUES
...
SELECT 'wallet-' || "id" || '-available', 'user:' || "id" || ':available:IC', "id", 'available', 'IC', "points_balance", 0, CURRENT_TIMESTAMP
FROM "users";

INSERT INTO "ledger_transactions" ("id", "idempotency_key", "type", "reference_type", "reference_id", "description")
SELECT 'opening-' || "id", 'wallet-opening:' || "id", 'opening_balance', 'user', "id", 'Migrated legacy points balance'
FROM "users" WHERE "points_balance" <> 0;

INSERT INTO "ledger_entries" ("id", "transaction_id", "account_id", "amount", "balance_after")
SELECT 'opening-user-' || u."id", 'opening-' || u."id", 'wallet-' || u."id" || '-available', u."points_balance", u."points_balance"
FROM "users" u WHERE u."points_balance" <> 0;

INSERT INTO "ledger_entries" ("id", "transaction_id", "account_id", "amount", "balance_after")
SELECT 'opening-system-' || u."id", 'opening-' || u."id", 'system-issuance-ic', -u."points_balance",
  -SUM(u."points_balance") OVER (ORDER BY u."id")
FROM "users" u WHERE u."points_balance" <> 0;

UPDATE "wallet_accounts"
SET "balance" = COALESCE((SELECT SUM(-u."points_balance") FROM "users" u), 0),
    "version" = (SELECT COUNT(*) FROM "users" WHERE "points_balance" <> 0),
